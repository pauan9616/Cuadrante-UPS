...
function getWorkingPeopleForDay(dayNumber) {
  const names = [];
  currentMonthDoc.data.sections.forEach((section) => {
    ...
    section.people.forEach((person, pi) => {
      ...
      if (WORKING_CODES.has(effective)) names.push(person.name);
    });
  });
...
function openPostAssign(dayNumber) {
  ...
  const working = getWorkingPeopleForDay(dayNumber);
  ...
  if (working.length === 0) {
    ...
    warn.className = 'hint';
    warn.textContent = 'No hay nadie con turno M/T/N ese día según el cuadrante.';
